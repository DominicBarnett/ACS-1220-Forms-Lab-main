...
class AuthorForm(FlaskForm):
    ...
    author_name = StringField('Author Name', 
        validators=[
            DataRequired(),
            Length(min=3, max=30, message="Please input the Author's name")
        ])
    # - the author's biography (hint: use a TextAreaField)
    author_bio = TextAreaField('Author Biography',
        validators=[
            DataRequired(),
            Length(min=10, max=500, message="Please input the Author's Biography")
        ])
    # - a submit button
    submit = SubmitField('Submit')
...
class GenreForm(FlaskForm):
    ...
    genre_name = StringField('Genre Name', 
        validators=[
            DataRequired(), 
            Length(min=3, max=20, message="Your message needs to be betweeen 3 and 80 chars")
        ])
    submit = SubmitField('Submit')
    # pass
